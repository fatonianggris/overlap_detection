...
Ie = adapthisteq(I);  %adapthisteq sebelum masuk segmentasi
It = im2bw(Ie,Th);
R = morphProcess(It,'square',6);

% BW = imbinarize(I,Th);
figure;
imshow(R)
pp = regionprops(R,'Area','Centroid');
amt = size(pp,1);
M = zeros(amt,2);

for i=1 : size(pp,1)
    M(i,:)= round(pp(i).Centroid);
end
end
% i need coordinates
